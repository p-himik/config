...
function pulseaudio:UpdateState()
    ...
    for sink, value in string.gmatch(out, "set%-sink%-volume ([^%s]+) (0x%x+)") do
        if sink == default_sink then
            self.Volume = tonumber(value) / max_volume
        end
    end

    -- convert volume to percentages of volume
    local v = self.Volume * 100
    self.Perc = math.floor(v) .. "%"
...
function pulseaudio:ToggleMute()
    local arg = self.Mute and 0 or 1
    run(cmd .. " set-sink-mute " .. default_sink .. " " .. arg)
    self:UpdateState()
end
