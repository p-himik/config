...
(local tag-specs (let [ls awful.layout.suit
                       t (fn [name layout classes]
                           {: name : layout : classes})]
                   ;; Table of layouts to cover with awful.layout.inc, order matters.
                   (awful.layout.append_default_layouts [ls.floating
                   ...
                   [(t :cmd ls.fair [])
                    ...
                    (t :dev ls.tile [:^jetbrains- :java-lang-Thread])
                    (t :soc ls.fair
                       [:Skype
                        :Telegram
                        :Slack
                    ...
                    (t :db ls.tile.left
                       [:com-install4j-runtime-launcher-Launcher
                        ...
                        :calibre])
                    (t "@" ls.tile.left [])
                    (t :vb ls.tile.left [:VirtualBox])
                    (t :8 ls.tile.right [:Deadbeef])
                    (t :9 ls.max.fullscreen [:Steam :steam])]))
    ...
    (when (and t.selected (= t.layout.name :centerwork))
      (let [find-client (fn []
        ...
        (when (?. chosen-client :focusable)
          (chosen-client:emit_signal "request::activate"
                                     :refocus-centerwork-layout-main-client)))))
