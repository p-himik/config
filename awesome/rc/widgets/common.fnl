...
(local awesome-menu [[:Hotkeys (fn [] (values false hotkeys-popup.show_help))]
            ...
            (let [name (or c.name "")
                  ...
                  cmd (fn []
                        (when c.valid
                          ...
                          (c:emit_signal "request::activate" :menu.clients
                                         {:raise true})))]
              (table.insert items {:text name : cmd :icon c.icon})))
          (when (not= nil (next items))
            (set instance (awful.menu.new {:theme {:width 300} : items}))
            (instance:show)
